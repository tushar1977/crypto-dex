...
@r.route("/", methods=["GET"])
def index():
    if "wallet_private_key" not in session and "wallet_address" not in session:
...
def create_import():
    ...
    if "create_wallet" in request.form:
        ...
        try:
            ...
            if not private_key or not acc_address:
        ...
        try:
            ...
            flash(f"Error: {str(e)}")
            return redirect(url_for("r.index"))
    elif "watch_wallet" in request.form:
        address = request.form.get("address")
        print(address)
        if not address:
            flash("Provide a correct address")
            return redirect(url_for("r.index"))
